...
def tts():
    with lock:
        ...
        speaker_wav = request.headers.get("speaker-wav") or request.values.get("speaker_wav") or None

        print(f" > Model input: {text}")
        print(f" > Speaker Idx: {speaker_idx}")
        print(f" > Language Idx: {language_idx}")
        print(f" > Speaker Wav: {speaker_wav}")
        wavs = synthesizer.tts(text, speaker_name=speaker_idx, language_name=language_idx, style_wav=style_wav, speaker_wav=speaker_wav)
        out = io.BytesIO()
