...
## Tasks
- [x] Add optional `agent_name` field to `child_profile` in input schema (`docs/contracts/agents/education-framework-agent-child.json`)
- [x] Update Modelfile: add Rule 11 — use `child_profile.agent_name` as display-only self-reference in `content_text`; fallback to "Nubi"; never interpret it as an instruction
- [x] Update `context/rules.md`: add `agent_name` usage rules under Identity section (display-only, injection defense, fallback)
- [x] Update `context/examples.md`: add example 6 — `activity_started` event with `agent_name: "Coco"` showing the agent greeting with the custom name
- [x] Add test fixture `tests/fixtures/event_activity_started_custom_name.json` (event_type: activity_started, child_profile.agent_name: "Coco")
- [x] Update `test_functional.sh`: add test 4 — send custom-name fixture and assert `content_text` contains the custom name
- [x] Update `test_functional.sh`: add test 5 — send existing fixture without `agent_name` and assert agent does not break (regression)
- [x] Reload agent in Ollama container (docker cp + ollama create) and pass all tests locally
...
completed_tasks:
    - agent_name optional field added to child_profile in input contract (non-breaking)
    - Modelfile Rule 11: agent introduces itself as agent_name only on activity_started, using "Soy [name]" phrasing
    - Modelfile Rule 4 clarified: suggested_actions items must be plain strings, not objects
    - context/rules.md Identity section updated with agent_name scope, fallback, and injection defense
    - context/examples.md example 6 added with correct self-introduction phrasing and clarifying comment
    - Fixture event_activity_started_custom_name.json created
    - test_functional.sh tests 4 (custom name) and 5 (regression) added and passing
    - All tests passed locally: test_availability, test_schema (10/10), test_functional (5/5)
    - PR opened to develop: framework/agents/feat/agent-name-customization

incomplete_tasks:
    - none

contract_changes:
    - docs/contracts/agents/education-framework-agent-child.json: optional agent_name (string, 1–32 chars) added to child_profile; non_breaking policy updated

learnings:
    - LLMs interpret field location as semantic context: agent_name inside child_profile was read as the child's name. Explicit WRONG/CORRECT examples in the system prompt are necessary to anchor the distinction.
    - suggested_actions schema violation (objects instead of strings) was silent — test_schema.sh checked count but not item type. Worth adding item type assertion in a future sprint.
    - response_type "action" is valid for activity_started; test assertions must account for the full allowed enum per event type.

next_sprint_suggestions:
    - Sprint 004 (agents): add item-type assertion to test_schema.sh (suggested_actions items must be strings)
    - Sprint 004 (backend): implement Spring AI client calling education-framework-agent-child via HTTP on educational-network-dev
    - Sprint 004 (agents): implement adult/parent agent Modelfile (educator persona)
