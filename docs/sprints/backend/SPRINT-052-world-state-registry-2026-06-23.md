...
### Registry
- [x] Add `WorldStateRegistry` port/interface with the methods listed in `Model Properties`.
- [x] Add `InMemoryWorldStateRegistry` backed by the map described in `Model Properties`.
- [x] Store and retrieve by `childSessionId`.
- [x] Enforce at most one active world state per child session.
- [x] Remove state when world session closes or child session ends.
- [x] Apply the behavior rules listed in `Model Properties`.

### Tests
- [x] Unit test save and find by child session id.
- [x] Unit test replacing or rejecting duplicate active state follows documented behavior.
- [x] Unit test remove closed state.
- [x] Unit test missing state returns empty result.
...
completed_tasks:
- Created `WorldStateRegistry` port interface in `world/ports/out/`
- Created `InMemoryWorldStateRegistry` implementation in `world/infrastructure/`
- Uses `ConcurrentHashMap` keyed by `childSessionId`
- `findByChildSessionId` returns only ACTIVE states (filters out CLOSED)
- `save` replaces existing state for same childSessionId (documented behavior)
- `removeByChildSessionId` returns removed state as Optional
- `clearClosed()` helper removes all CLOSED states
- Created `InMemoryWorldStateRegistryTest` with 10 unit tests
- All 750 tests pass
...
contract_changes:
- None (internal in-memory infrastructure only)

learnings:
- WorldStateRegistry is simpler than GameStateRegistry because world only needs childSessionId lookup (no secondary gameId index needed)
- Filtering by ACTIVE status in findByChildSessionId ensures closed states are not returned

next_sprint_suggestions:
- Sprint 053: World engagement threshold config